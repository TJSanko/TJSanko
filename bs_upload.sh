...
function help(){
  ...
  echo -e "           t         number of threads to use (default: t=2)"
...
if [[ ${SERVER} == "" || ${SERVER} == .$ || ${SRV[$SERVER]} -eq 0 ]]; then eval $(~/.local/bscmd load config); fi;
if [[ ${SRV[$SERVER]} -eq 1 ]]; then eval $(~/.local/bscmd load config eu); fi;
...
if [[ ${THR} -eq 0 || -z ${THR} || ${THR} == ^$ || ${THR} != \d+ ]]; then THR=2; fi;
